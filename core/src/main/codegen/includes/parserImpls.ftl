...
SqlCreate SqlCreateTable(Span s, boolean replace) :
{
    ...
    SqlNode query = null;
    SqlNode tableNameInfo;
}
{
    <TABLE> ifNotExists = IfNotExistsOpt() id = CompoundIdentifier()
    {
        SqlLiteral tableNameKey = SqlLiteral.createCharString("create_target_table", s.end(id));
        SqlLiteral tableNameLiteral = SqlLiteral.createCharString(token.image, s.end(id));
        tableNameInfo = SqlStdOperatorTable.CONCAT.createCall(s.end(id), tableNameLiteral, tableNameKey);
    }
    ...
    [
        (
            LOOKAHEAD(2)
            <AS> <RUN>
            query = parseCreateSubQuery(tableNameInfo)
            {
                return new SqlCreateTableFromModel(s.end(this), replace, ifNotExists, id, SqlNodeList.EMPTY, query);
            }
